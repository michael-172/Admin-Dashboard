...
export const getUsers = createAsyncThunk('users/getUsers', async (limitNumber, thunkAPI) => {
    ...
    try{
        const res = (limitNumber) ? await fetch(`http://localhost:5000/users/?_limit=${limitNumber}`) : await fetch(`http://localhost:5000/users`);
...
export const deleteUser = createAsyncThunk('users/deleteUser', async (item, thunkAPI) => {
    const {rejectWithValue} = thunkAPI;
    try{
        await fetch(`http://localhost:5000/users/${item.id}`, {
            method: 'DELETE',
            headers: {
            'Content-type': 'application/json; charset=UTF-8',
            },
